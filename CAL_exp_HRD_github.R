...
########running time
args = c('1','./TCGA_TNBC_DEG_test.ENSG.drop.csv', 'OUT_TABLE.csv', 'OUT_plot_DEG.png', 'confidence', 0.95)
...
read_RNA <- function(option){
  #print(option)
  if (option == "0"){ # deseq2
    data <- read.csv('./TCGA_OV_DEG_test.ENSG.drop.csv', header = TRUE,sep =',',  row.names = 1, check.names = FALSE)
  }
  #print(head(data))
  return(data)
}
...
gene_set_before <- readLines('./FINAL_98_GENE_ENSG_ID_repo.csv')
...
hrd_score <- read.csv('./OV_HRD.csv', row.names = 1, check.names = F) # pre-calculated HRD score
# do not changed
